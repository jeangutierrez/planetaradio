...
(function ($) {
    ...
    var _ = {
        ...
        init: function (options) {
            ...
            var template = ['<svg viewBox="0 0 100 100" id="playable" version="1.1" xmlns="http://www.w3.org/2000/svg" width="60" height="60" data-play="playable" class="not-started playable">',
                '<g class="shape">',
                '<circle class="progress-track" cx="50" cy="50" r="47.45" stroke="#F0F0F0" stroke-opacity="0.25" stroke-linecap="round" fill="none" stroke-width="2"/>',
                '<circle class="precache-bar" cx="50" cy="50" r="47.45" stroke="#F0F0F0" stroke-opacity="0.25" stroke-linecap="round" fill="none" stroke-width="2" transform="rotate(-90 50 50)"/>',
                '<circle class="progress-bar" cx="50" cy="50" r="47.45" stroke="#F0F0F0" stroke-opacity="0.25" stroke-linecap="round" fill="none" stroke-width="2" transform="rotate(-90 50 50)"/>',
                '</g>',
                '<circle class="controls" cx="50" cy="50" r="45" stroke="none" fill="#F0F0F0" opacity="0.0" pointer-events="all"/>',
                '<g class="control pause" style="filter:drop-shadow(0 0 3px white);">',
                '<line x1="40" y1="35" x2="40" y2="65" stroke="#F0F0F0" fill="none" stroke-width="8" stroke-linecap="round"/>',
                '<line x1="60" y1="35" x2="60" y2="65" stroke="#F0F0F0" fill="none" stroke-width="8" stroke-linecap="round"/>',
                '</g>',
                '<g class="control play" style="filter:drop-shadow(0 0 3px white)">',
                '<filter id="shadow" x="-20%" y="-20%" width="200%" height="200%">',
                '<feDropShadow dx="20" dy="3" stdDeviation="5" flood-color="#5091b9" />',
                '</filter>',
                '<polygon points="40,30 70,50 40,70" fill="#F0F0F0" stroke-width="0" filter="drop-shadow( 6px 0 2px hsla(0, 0%, 0%, 0.2))"></polygon>',
                '</g>',
                '<g class="control stop">',
                '<rect x="35" y="35" width="30" height="30" stroke="#F0F0F0" fill="#F0F0F0" stroke-width="0"/>',
                '</g>',
                '</svg>'];
